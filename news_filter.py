"""
News Filter Module - Calendrier Économique
===========================================
Filtre les trades autour des événements économiques majeurs:
...
Règles:
1. Pas de nouvelles positions 30 min avant high-impact news
2. Fermer les positions ou élargir les stops avant news majeures
3. Éviter de trader pendant les releases
"""
import logging
...
class NewsFilter:
    """
    Filtre les trades basé sur le calendrier économique.
    ...
    def _load_recurring_events(self) -> List[Dict]:
        ...
        return [
            # USD Events
            {
                ...
                'day_of_month': 'first_friday',  # First Friday of month
                'time_utc': '13:30'
            },
            {
                'currency': 'USD',
                ...
                'impact': NewsImpact.CRITICAL,
                'frequency': 'fomc_schedule',  # 8 times per year
                'time_utc': '19:00'
            },
            {
                'currency': 'USD',
                ...
                'day_of_month': 'mid_month',  # Around 13th
                'time_utc': '13:30'
            },
            {
                'currency': 'USD',
            ...
            # EUR Events
            {
                ...
                'impact': NewsImpact.CRITICAL,
                'frequency': 'monthly',  # Usually first Thursday
                'time_utc': '13:15'
            },
            {
                'currency': 'EUR',
    ...
    def get_upcoming_events(self, hours_ahead: int = 24) -> List[EconomicEvent]:
        ...
        # Simplified: Return events scheduled for today/tomorrow
        # In production, this would query a live API
        upcoming = []

        # Check if today is a significant news day
        today = now.date()
        ...
        for event_template in self.recurring_events:
            ...
            # Add event if within time window
    ...
    def should_trade(self, pair: str) -> Tuple[bool, str, Optional[EconomicEvent]]:
        ...
        for event in upcoming:
            ...
            time_to_event = (event.timestamp - now).total_seconds() / 60  # in minutes

            # Check buffers based on impact
    ...
    def get_risk_adjustment(self, pair: str) -> float:
        ...
        Returns:
            ...
            - 1.0 = normal risk
            - 0.5 = half risk (news approaching)
            - 0.0 = no trading (too close to news)
        """
        ...
        # Check for events in wider window
        currencies = self._get_currencies_from_pair(pair)
        ...
        for event in upcoming:
            ...
            # Gradual risk reduction as news approaches
            if event.impact == NewsImpact.CRITICAL:
                if time_to_event <= 120:  # Within 2 hours
                    risk = 0.3 + (time_to_event / 120) * 0.7
                    min_risk = min(min_risk, risk)

            elif event.impact == NewsImpact.HIGH:
                if time_to_event <= 60:  # Within 1 hour
    ...
    def get_news_summary(self, pair: str = None) -> Dict:
        ...
        return {
            ...
            'events': [
                ...
                for e in upcoming[:10]  # Limit to next 10 events
            ]
        }
