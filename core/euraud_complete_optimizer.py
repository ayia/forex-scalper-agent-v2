#!/usr/bin/env python3
"""
EUR/AUD Complete Strategy Optimizer V1.0
=========================================
Complete strategy optimization system for EUR/AUD.
Tests 40+ strategies, optimizes parameters, validates with Monte Carlo & Walk-Forward.

EUR/AUD Characteristics:
- Medium-high volatility (higher than EUR/GBP, similar to EUR/JPY)
- pip_value = 0.0001 (4 decimal pair)
- Average daily range: 80-120 pips
- Best sessions: London overlap with Sydney/Tokyo, London session
- Correlates with: AUD/USD (inverse), EUR/USD, commodity prices (inverse)
- Commodity currency influence: Gold, Iron Ore prices affect AUD
...
# =============================================================================
# MACRO PERIODS FOR TESTING - EUR/AUD SPECIFIC
# =============================================================================
...
MACRO_PERIODS = {
    'COVID_CRASH': {
        'start': '2020-03-01',
        'end': '2020-04-30',
        'description': 'COVID-19 Market Crash - AUD collapsed then recovered',
    ...
    'COVID_RECOVERY': {
        'start': '2020-05-01',
        'end': '2020-12-31',
        'description': 'Post-COVID Recovery - AUD strength on commodity boom',
    ...
    'COMMODITY_BOOM_2021': {
        'start': '2021-01-01',
        'end': '2021-12-31',
        'description': 'Iron Ore & Commodity Supercycle - AUD strong',
        'expected_regime': 'TRENDING_DOWN',  # EUR/AUD falling as AUD strengthens
        'priority': 'OBLIGATOIRE'
    },
    'FED_HIKING': {
        'start': '2022-03-01',
        'end': '2023-06-30',
        'description': 'Fed Aggressive Rate Hikes - Risk-off, AUD weak',
        'expected_regime': 'TRENDING_UP',  # EUR/AUD rising
        'priority': 'OBLIGATOIRE'
    },
    'CHINA_SLOWDOWN': {
        'start': '2022-06-01',
        'end': '2023-06-30',
        'description': 'China Economic Slowdown - AUD weak (China is major AUD trading partner)',
        'expected_regime': 'HIGH_VOLATILITY',
        'priority': 'OBLIGATOIRE'
    },
    'RBA_HIKING': {
        'start': '2022-05-01',
        'end': '2023-12-31',
        'description': 'RBA Rate Hiking Cycle - AUD supported',
    ...
    'BANKING_CRISIS': {
        'start': '2023-03-01',
        'end': '2023-05-31',
        'description': 'SVB/Credit Suisse Crisis - Risk-off',
        'expected_regime': 'HIGH_VOLATILITY',
        'priority': 'OBLIGATOIRE'
    },
    'RATE_DIVERGENCE_2024': {
        'start': '2024-01-01',
        'end': '2024-06-30',
        'description': 'ECB vs RBA Rate Divergence',
        'expected_regime': 'RANGING',
        'priority': 'OBLIGATOIRE'
    },
    'NORMAL_2019': {
        'start': '2019-01-01',
        'end': '2019-12-31',
        'description': 'Normal Market Baseline (pre-COVID)',
...
class Indicators:
    """Technical indicator calculations optimized for EUR/AUD."""
...
class StrategyLibrary:
    """Library of 40 trading strategies optimized for EUR/AUD."""
    ...
    def _calculate_all_indicators(self):
        ...
        # ATR - EUR/AUD has medium-high ATR
    ...
    def strategy_macd_stochastic(self, params: Dict = None) -> pd.Series:
        """MACD + Stochastic: Double confirmation - OPTIMIZED FOR EUR/AUD."""
        df = self.df
...
# =============================================================================
# BACKTESTER FOR EUR/AUD
# =============================================================================

class Backtester:
    """Backtesting engine optimized for EUR/AUD (pip = 0.0001)."""

    def __init__(self, df: pd.DataFrame, pip_value: float = 0.0001):
        """
        Initialize backtester.

        EUR/AUD pip value = 0.0001 (100 pips = 0.01 move)
        """
        self.df = df
    ...
    def run(self, signals: pd.Series, strategy_name: str,
        ...
        for i in range(1, len(df)):
            ...
            if position is None and row['signal'] != 0:
                entry = row['close']
                atr = row['atr'] if not pd.isna(row['atr']) else 0.0080  # Default ATR for EUR/AUD ~80 pips
...
class RegimeDetector:
    """Detect market regimes for EUR/AUD."""
...
class MonteCarloSimulator:
    ...
    def run(self, n_simulations: int = 500, initial_capital: float = 10000) -> Dict:
        ...
        for _ in range(n_simulations):
            ...
            for pnl in shuffled:
                # Scale PnL for lot size (0.1 lot = ~$1 per pip for EUR/AUD)
                equity.append(equity[-1] + pnl * 10000)  # EUR/AUD pnl is in price units
...
class WalkForwardOptimizer:
    ...
    def run(self, strategy_func: Callable, strategy_name: str,
            n_segments: int = 5, train_pct: float = 0.7) -> Dict:
        """Run walk-forward optimization."""
        segment_size = len(self.df) // n_segments
        ...
        for i in range(n_segments):
            ...
            train_size = int(len(segment_df) * train_pct)

            train_df = segment_df.iloc[:train_size]
            test_df = segment_df.iloc[train_size:]
            ...
            try:
                # Train period
                library = StrategyLibrary(train_df)
                strategies = library.get_all_strategies()
                signals = strategies[strategy_name]()
                backtester = Backtester(train_df, self.pip_value)
                train_result = backtester.run(signals, strategy_name)

                # Test period
                library = StrategyLibrary(test_df)
                strategies = library.get_all_strategies()
                signals = strategies[strategy_name]()
                backtester = Backtester(test_df, self.pip_value)
                test_result = backtester.run(signals, strategy_name)

                if train_result.total_trades >= 10 and test_result.total_trades >= 5:
                    in_sample_pfs.append(train_result.profit_factor)
                    out_sample_pfs.append(test_result.profit_factor)

            except Exception:
                continue

        if not in_sample_pfs or not out_sample_pfs:
            return {
                'wfe': 0,
                'in_sample_mean': 0,
                'out_sample_mean': 0,
        ...
        in_sample_mean = np.mean(in_sample_pfs)
        out_sample_mean = np.mean(out_sample_pfs)
        wfe = out_sample_mean / in_sample_mean * 100 if in_sample_mean > 0 else 0

        return {
            'wfe': round(wfe, 1),
            'in_sample_mean': round(in_sample_mean, 2),
            'out_sample_mean': round(out_sample_mean, 2),
            'segments_tested': len(in_sample_pfs),
            'passed': wfe >= 50
        }


# =============================================================================
# MAIN OPTIMIZER FOR EUR/AUD
# =============================================================================

class EURAUDOptimizer:
    """Main optimizer for EUR/AUD strategies."""
    ...
    def __init__(self, verbose: bool = True):
        ...
        self.results = []
        self.pip_value = 0.0001  # EUR/AUD

    def log(self, msg: str):
        if self.verbose:
            print(msg)

    def fetch_data(self, start: str = '2019-01-01', end: str = None) -> Optional[pd.DataFrame]:
    ...
    def optimize_parameters(self, df: pd.DataFrame, strategy_name: str,
        ...
        # EUR/AUD specific parameter grid
    ...
    def run_walk_forward(self, df: pd.DataFrame, strategy_name: str) -> Dict:
        """Run Walk-Forward Optimization."""
        self.log(f"\n{'='*70}")
        self.log(f"[STEP 6b/8] RUNNING WALK-FORWARD OPTIMIZATION")
        self.log(f"{'='*70}")

        wfo = WalkForwardOptimizer(df, self.pip_value)

        library = StrategyLibrary(df)
        strategies = library.get_all_strategies()

        results = wfo.run(strategies[strategy_name], strategy_name)

        self.log(f"\nWalk-Forward Results ({results['segments_tested']} segments):")
        self.log(f"   Walk-Forward Efficiency: {results['wfe']:.1f}%")
        self.log(f"   In-Sample Mean PF: {results['in_sample_mean']:.2f}")
        self.log(f"   Out-of-Sample Mean PF: {results['out_sample_mean']:.2f}")
        self.log(f"   Validation: {'PASS' if results['passed'] else 'NEEDS REVIEW'}")
    ...
    def validate_risk(self, trades: List[Trade], max_daily_loss: float = 500) -> Dict:
        ...
        for trade in trades:
            ...
            if day not in daily_pnl:
                daily_pnl[day] = 0
            # EUR/AUD: 0.1 lot = ~$10/pip
            daily_pnl[day] += trade.pnl_pips * 10
    ...
    def test_macro_periods(self, strategy_name: str, strategy_func: Callable) -> Dict:
        """Test strategy on all macro periods."""
        self.log(f"\n{'='*70}")
        self.log(f"[STEP 7b/8] TESTING MACRO PERIODS")
        self.log(f"{'='*70}")
        ...
        for period_name, period_info in MACRO_PERIODS.items():
            ...
            df = self.fetcher.fetch(self.pair, period_info['start'], period_info['end'], '1d')
            ...
            try:
                ...
                backtester = Backtester(df, self.pip_value)
                result = backtester.run(signals, strategy_name, rr=2.0, sl_atr_mult=1.5)
                ...
                if result.total_trades >= 3:
                    ...
                    self.log(f"      {status} PF={result.profit_factor:.2f}, Trades={result.total_trades}")
                else:
                    self.log(f"      Too few trades ({result.total_trades})")
                    period_results[period_name] = None
    ...
    def generate_report(self, best_strategy: BacktestResult, params: Dict,
                       regime_stats: Dict, monte_carlo: Dict, risk: Dict,
                       wfo: Dict = None, macro_periods: Dict = None) -> str:
        """Generate final report."""
        self.log(f"\n{'='*70}")
        self.log(f"[STEP 8/8] GENERATING FINAL REPORT")
        self.log(f"{'='*70}")

        # Determine tradeable/avoid regimes based on EUR/AUD characteristics
        tradeable_regimes = ['TRENDING_UP', 'TRENDING_DOWN', 'STRONG_TREND_UP', 'RANGING']
...
EUR/AUD CHARACTERISTICS:
  - pip_value: 0.0001 (100 pips = 0.01 move)
  - Average Daily Range: 80-120 pips
...
WALK-FORWARD VALIDATION:
  - Walk-Forward Efficiency: {wfo['wfe']:.1f}%
  - In-Sample Mean PF: {wfo['in_sample_mean']:.2f}
  - Out-of-Sample Mean PF: {wfo['out_sample_mean']:.2f}
  - Status: {'PASS' if wfo['passed'] else 'NEEDS REVIEW'}
"""
...
"""
            for period, data in macro_periods.items():
                if data:
                    status = "[OK]" if data['pf'] >= 0.8 else "[--]"
                    report += f"  {status} {period}: PF={data['pf']:.2f}, Trades={data['trades']}\n"
                else:
                    report += f"  [--] {period}: Insufficient data\n"

        report += f"""
SESSION RECOMMENDATIONS:
  [OK] LONDON: 07:00-16:00 UTC (Best liquidity for EUR)
  [OK] SYDNEY-LONDON OVERLAP: 06:00-08:00 UTC (Good for AUD movements)
  [--] ASIAN: 00:00-07:00 UTC (Lower EUR liquidity)
  [--] NEW YORK: 12:00-21:00 UTC (May work for EUR movements)

EXPECTED PERFORMANCE:
  - Profit Factor: {best_strategy.profit_factor:.2f}
  - Win Rate: {best_strategy.win_rate:.1f}%
  - Monthly Trades: ~{int(best_strategy.total_trades / 12) if best_strategy.total_trades > 12 else best_strategy.total_trades}
  - Max Drawdown: {best_strategy.max_drawdown:.1f}%
...
"""
    ...
    def save_results_to_csv(self, all_results: List[BacktestResult], filename: str = None):
        ...
        if filename is None:
            filename = f"euraud_optimization_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def run(self, start_date: str = '2019-01-01'):
        ...
        print("=" * 70)
        print("EUR/AUD STRATEGY OPTIMIZER V1.0")
        print("=" * 70)
        ...
        # 6b. Walk-Forward Optimization
        wfo_results = self.run_walk_forward(df, final_result.strategy_name)

        # 7. Risk validation
        risk_results = self.validate_risk(final_result.trades)

        # 7b. Macro period testing
        macro_results = self.test_macro_periods(final_result.strategy_name,
                                                strategies[final_result.strategy_name])

        # 8. Generate report
        final_params = opt_result['best_params'] if opt_result['best_params'] else {}
        report = self.generate_report(final_result, final_params, regime_stats,
                                      mc_results, risk_results, wfo_results, macro_results)
        print(report)
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("STARTING EUR/AUD COMPLETE STRATEGY OPTIMIZATION")
    print("=" * 70)

    optimizer = EURAUDOptimizer(verbose=True)
    result = optimizer.run(start_date='2019-01-01')
