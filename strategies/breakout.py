...
class BreakoutStrategy(BaseStrategy):
    ...
    def generate_signal(self, df: pd.DataFrame, pair: str, timeframe: str) -> Optional[Signal]:
        ...
        """
        result = self.analyze_df(df, pair)
        if result:
    ...
    def analyze(self, data: Dict[str, pd.DataFrame], pair: str) -> Optional[Signal]:
        """Analyze from dict of DataFrames (BaseStrategy interface)."""
        if "M15" not in data:
            return None
        return self.generate_signal(data["M15"], pair, "M15")

    def analyze_df(self, df: pd.DataFrame, pair: str) -> Optional[Dict]:
        """
